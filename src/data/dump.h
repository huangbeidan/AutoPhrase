...
//    calculate three scores: top 5% average, % with score > 0.9, percentage of middle-zone scores
    ...
    vector<pair<double, PATTERN_ID_TYPE>> order;
    int one_score_patterns=0;
    int two_score_patterns=0;
    int three_score_patterns=0;
    int four_score_patterns=0;
    int five_score_patterns=0;

    for (PATTERN_ID_TYPE i = 0; i < patterns.size(); ++ i) {
        if (patterns[i].size() > 1 && patterns[i].currentFreq > 0) {
            order.push_back(make_pair(patterns[i].quality, i));
            if ((double) patterns[i].quality > 0.9){
                one_score_patterns += 1;
            }else if((double) patterns[i].quality > 0.8){
                two_score_patterns += 1;
            } else if((double) patterns[i].quality > 0.7){
                three_score_patterns += 1;
            } else if((double) patterns[i].quality > 0.6){
                four_score_patterns += 1;
            }
        }
    }
...
//    fprintf(out, "% with 0.8 > score > 0.7 is: %.10f\n", (double ) middle_score_patterns/size);
    fprintf(out, "% with score>0.9 is: %.10f\n", (double) one_score_patterns/size);
    fprintf(out, "% with score>0.8 is: %.10f\n", (double) two_score_patterns/size);
    fprintf(out, "% with score>0.7 is: %.10f\n", (double) three_score_patterns/size);
    fprintf(out, "% with score>0.6 is: %.10f\n", (double) four_score_patterns/size);
    fprintf(out, "\n");
